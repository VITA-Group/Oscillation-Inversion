...
def main(file_path_blur, file_path_blend, prompt_input, idx, save_path, centeridx):
    ...
    with torch.no_grad(): 
        ...
        for b in range(30):
            ...
            #image[0].save(f"/home/zy3724/git_repos/flatten/video_samples/talkman/optimize/pre60_{b:08d}_{timestamp}.png")
        ...
        latents_pt.append(V_mean)
        torch.save(latents_pt, "/home/zy3724/git_repos/flatten/video_samples/talkman/latents_pt.pt")
        ...
        latent_ = pipe._unpack_latents(V_predict_mean, height, width, pipe.vae_scale_factor)
        latent_ = (latent_ / pipe.vae.config.scaling_factor) + pipe.vae.config.shift_factor
        image = pipe.vae.decode(latent_, return_dict=False)[0]
        image = pipe.image_processor.postprocess(image, output_type=output_type)
    ...
    #         if centeridx == 3:
    #             V_even = V_history[3::6] 

    #         if centeridx == 4:
    #             V_even = V_history[4::6] 

    #         if centeridx == 5:
    #             V_even = V_history[5::6] 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--folder_path_blur', type=str, required=False, help="Path to the blurred image file")
    parser.add_argument('--folder_path_blend', type=str, required=False, help="Path to the blended image file")
    parser.add_argument('--prompt_input', type=str, required=False, help="Prompt for image generation")
    parser.add_argument('--idx', type=int, required=False, help="Index parameter")
    parser.add_argument('--save_path', type=str, required=False, help="Path to save the resulting image")
    parser.add_argument('--centeridx', type=int, required=False, help="Center index for processing")

    args = parser.parse_args()

    args.folder_path_blur = "/home/zy3724/git_repos/flatten/video_samples/talkman/pis_enhanced"
    args.folder_path_blend = "/home/zy3724/git_repos/flatten/video_samples/talkman/mag_enhanced"
    ...
    blur_files = []
    blend_files = []
    # Loop through the files from 00000000.png to 00000047.png
    for i in range(48):
        filename = f"{i:08d}.png"
        blur_path = os.path.join(args.folder_path_blur, filename)
        blend_path = os.path.join(args.folder_path_blend, filename)
        
        # Check if both files exist
        if os.path.exists(blur_path) and os.path.exists(blend_path):
            blur_files.append(blur_path)
            blend_files.append(blend_path)
        else:
            print(f"Warning: File {filename} not found in both directories")
    # Now blur_files and blend_files contain the paths to corresponding images
    print(f"Found {len(blur_files)} matching image pairs")
    # You can now use these lists in your main function call
    for i, (blur_file, blend_file) in enumerate(zip(blur_files, blend_files)):
        args.file_path_blur = blur_file
        args.file_path_blend = blend_file
        args.idx = 60
        args.save_path = f"/home/zy3724/git_repos/flatten/video_samples/talkman/optimize/pre60_{i:08d}.png"  # Adjust this path as needed
        args.prompt_input = "a side view of real talking man face, open mouth and show teech a little bit, high quality, 4K"  # Set a default prompt if needed
        args.centeridx = 0  # Set a default centeridx if needed
        # Call the main function for each pair of images
