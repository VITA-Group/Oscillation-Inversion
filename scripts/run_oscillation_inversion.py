...
from src.flux_utils import flux_optim
...
for r in range(reoptim_iters):
    ...
    print(pils_source)
    print(pils_target)
    ipviews_edited_images = flux_optim( pils_source , pils_target, text_prompt = 'wood growth rings, no letter, no number', optim_iters = optim_iters, pils_sde = None )
...
grid = make_image_grid(edited_sequence, save_path=save_dir, row_width = optim_iters, name = f'{name}65_dev')
